...
export async function getTeacherSectionSessionsService(paramObj, projection={}, populateObj=""){
    try {
        const teacherSectionSessions = await teacherSectionSessionModel.find(paramObj).select(projection).populate(populateObj);
        return teacherSectionSessions;
    } catch (error) {
        throw error;
    }
