...
import {
  checkHolidayEventExist,
  createHolidayEvent,
  deleteHolidayEventById,
  getEventList,
  getHolidayEventById,
  updateHolidayEvent
} from "../services/holidayEvent.service.js";
...
export async function createHolidayEventController(req, res) {
  try {
    ...
    const holidayEvent = await checkHolidayEventExist({
      adminId,
      startOfDay,
      endOfDay
    });
    ...
    const createdHolidayEvent = await createHolidayEvent({
      currDate,
      day,
      title,
      holiday,
...
export async function updateHolidayEventController(req, res) {
  try {
    ...
    const updatedHolidayEvent = await updateHolidayEvent({
      eventId,
      title,
      description,
      holiday,
