  ...
  try {
    ...
    if (!session || session["status"] === "completed") {
      return res
        ...
        .send(error(404, "Session completed! Can't mark attendance"));
    }
    if (!section) {
      return res
    ...
    if (date < session.startDate || date > session.endDate) {
      return res
        .status(StatusCodes.BAD_REQUEST)
        .send(error(400, "Can't mark attendance outside session duration"));
    }
    ...
    date = date.getTime();
    if (day === "Sunday") {
      const sundayAsWorkDay = await getWorkDayService({
    ...
    for (const sessionStudent of present) {
      try {
        const paramObj = {
          sessionStudent: sessionStudent["id"],
          date: { $gte: startTime, $lte: endTime },
          parentAttendance: { $ne: "" }
        };
        ...
        if (parentMarkedAttendance) {
          ...
          const attendanceObj = {
            ...
            day,
            sessionStudent: sessionStudent["id"],
            student: storedSessionStudent["student"],
            teacherAttendance: "present",
            section: sectionId,
            classId: section["classId"],
...
export async function undoAttendanceByTeacherController(req, res) {
  ...
  if (!session || session["status"] === "completed") {
    return res
      .status(StatusCodes.BAD_REQUEST)
      .send(error(404, "Session completed! Can't undo attendance"));
  }
  const { startTime, endTime } = getStartAndEndTimeService(date, date);
  const sectionAttendance = await getSectionAttendanceService({
    section: sectionId,
        ...
        .send(
          error(
            ...
            attendanceControllerResponse.attendanceByParentController
              .attendanceAlreadyMarkedByParent
          )
        );
    }

    const markAttendance = await createAttendanceService({
      student: studentId,
      date,
      day,
  ...
  try {
    ...
    for (let attendanceTimestamp in studentsAttendances) {
      ...
      for (const studentAttendance of studentsAttendances[attendanceTimestamp]) {
        ...
        const existingAttendance = await getAttendanceService({
          sessionStudent: studentAttendance["sessionStudent"],
          date: { $gte: startTime, $lte: endTime }
        });
        if (existingAttendance) {
          await updateAttendanceService(
            { _id: existingAttendance["_id"] },
            { teacherAttendance: studentAttendance["attendance"] }
          );
        } else {
          const attendanceObj = {
            date: attendanceTimestamp,
            day: dayName,
            sessionStudent: studentAttendance["sessionStudent"],
            teacherAttendance: studentAttendance["attendance"],
            section: sectionId,
...
export async function updateAttendanceController(req, res) {
  try {
    ...
    if (!session || session["status"] === "completed") {
      return res
        .status(StatusCodes.BAD_REQUEST)
        .send(error(404, "Session completed! Can't update attendance"));
    }
    ...
    sectionAttendance["presentCount"] =
      sectionAttendance["presentCount"] - absentLength + presentLength;
    sectionAttendance["absentCount"] =
      sectionAttendance["absentCount"] - presentLength + absentLength;
...
export async function checkAttendaceMarkedController(req, res) {
  try {
    ...
    if (section["guestTeacher"] && req.role === "teacher") {
      return res
        .status(StatusCodes.UNAUTHORIZED)
        .send(error(401, "Teacher is not authorized for attendance"));
    }
    let date = new Date();
    const { startTime, endTime } = getStartAndEndTimeService(date, date);
...
export async function getAttendancesController(req, res) {
  try {
    ...
    if (sessionStudent) {
      filter["_id"] = convertToMongoId(sessionStudent);
    }
    if (section) {
