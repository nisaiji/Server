...
export async function sendNotificationByAdminController(req, res) {
  try {
    ...
    const res1 = await sendPushNotification(tokenApp, title, description, "notification", req?.adminId);
    const res2 = await sendPushNotification(tokenWeb, title, description, "notification", req?.adminId);
