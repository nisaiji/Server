...
import {
  ...
  registerGuestTeacherService
} from "../services/guestTeacher.service.js";
import {
  getLeaveRequestsCountService,
  getLeaveRequestService,
  getLeaveRequestsPipelineService,
  registerLeaveRequestService,
...
export async function registerLeaveRequestController(req, res) {
  try {
    ...
    await sendPushNotification(
      req.fcmToken,
      "Leave Request",
      `Your leave request from ${getFormattedDateService(new Date(startTime))} to ${getFormattedDateService(new Date(endTime))} has been sent successfully`,
      "leaveRequest",
      req?.teacherId
    );
  ...
  try {
    ...
    if (status === "accept") {
      ...
      if (existingTeacher || existingGuestTeacher) {
        return res
          .status(StatusCodes.CONFLICT)
          .send(success(409, "Username already exists. Try a different one"));
      }
      const guestTeacherObj = {
        username,
      ...
      await updateTeacherService(
        { _id: teacher["_id"] },
        { leaveRequestCount: teacher["leaveRequestCount"] + 1 }
      );
    }
