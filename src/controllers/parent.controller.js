...
export async function registerParentController(req, res) {
    try {
     ...
     if(existingParent){
      return res.send(error(400,"student is already linked with parent, Cann't create parent again!"));
     }
...
export async function registerExistingParentController(req, res) {
    try {
      ...
      const isChildLinkedToAnotherParent = await checkStudentAlreadyLinkedToParent(studentId);
      if(isChildLinkedToAnotherParent){
        return res.send(error(400,"child is already linked with another parent"));
      }
