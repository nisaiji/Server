...
const teacherUpdationSchema = Joi.object({
  ...
  username: Joi.string().min(5).max(15).optional().messages({
    "string.min": "Username must be at least 5 characters.",
    "string.max": "Username must be at most 15 characters.",
    "any.required": "Username is required.",
  }),
  ...
  gender: Joi.string()
    .valid("Female", "Male", "Other", "Prefer not to say")
    .optional()
    .messages({
      "any.only": "Invalid gender format.",
    }),
  university: Joi.string().optional().messages({
    "string.base": "Invalid university format.",
  }),
  degree: Joi.string().optional().messages({
    "string.base": "Invalid degree format.",
  }),
  dob: Joi.string().optional().messages({
    "string.base": "Invalid dob format.",
  }),
});
