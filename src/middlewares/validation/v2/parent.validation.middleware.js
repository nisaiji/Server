...
export async function parentPasswordEditValidation(req, res, next) {
  try {
    const { error: schemaError } = parentPasswordEditValidator.validate(
      req.body
    );
    if (schemaError) {
      return res
...
export async function parentPhoneAndOtpValidation(req, res, next) {
  try {
    const { error: schemaError } = parentPhoneAndOtpValidator.validate(
      req.body
    );
    if (schemaError) {
      return res
...
export async function parentPhotoUploadValidation(req, res, next) {
  try {
    const { error: schemaError } = uploadParentPhotoValidator.validate(
      req.body
    );
    if (schemaError) {
      return res
