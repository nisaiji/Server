...
import { parentAuthUpdateSchema, parentLoginSchema, parentPasswordChangeSchema, parentProfileInfoUpdateSchema, parentProfileUpdateSchema } from "../../validators/parentSchema.validator.js";
...
export async function passwordChangeParentValidation(req, res, next) {
  try {
    const { error: schemaError } = parentPasswordChangeSchema.validate(req.body);
    if (schemaError) {
      return res.status(StatusCodes.BAD_REQUEST).send(error(400, schemaError.details[0].message));
    }
    next()
