...
export async function profileInfoUpdateParentValidation(req, res, next) {
  try {
    const { error: schemaError } = parentProfileInfoUpdateSchema.validate(
      req.body
    );
    if (schemaError) {
      return res
...
export async function AddressUpdateParentValidation(req, res, next) {
  try {
    const { error: schemaError } = parentAddressUpdateSchema.validate(req.body);
    if (schemaError) {
      return res
        .status(StatusCodes.BAD_REQUEST)
        .send(error(400, schemaError.details[0].message));
...
export async function passwordChangeParentValidation(req, res, next) {
  try {
    const { error: schemaError } = parentPasswordChangeSchema.validate(
      req.body
    );
    if (schemaError) {
      return res
